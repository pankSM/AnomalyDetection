...
# 这部分剪切代码主要用于剪切MAD
def CropMAD(imageName=None, savePath=None, pattern=".png", cropSize=(64, 64), stepSize=64, isTrain=True):
    """
    ...
    while(startRow + cropSize[0] <= h):
        ...
        # 如果列越界
        if(startCol + cropSize[1] > w and startCol < w):
...
def main():
    ...
    for i in tqdm(range(len(imageFiles))):
        file = imageFiles[i]
        CropMAD(imageName=file, savePath=dst_path, pattern=".png", cropSize=cropSize, stepSize=stepSize)
